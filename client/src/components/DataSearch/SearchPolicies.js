 ...
 * SearchPolicies.js
 * 
 * This is a React component that allows the user to search for policies by property. It includes a search bar, a clear button, and a paginated display of the search results.
 * 
 ...
 * 
 * The search term, page number, search results, and loading state are managed by the state. When the search button is clicked, the `/policies/search/${searchTerm}?page=${page}` endpoint is called to fetch the search results. The search results are displayed in a grid, with each policy being a link to its detail page.
 * 
 ...
 * 
 * @module components/SearchPolicies
 */
...
const SearchPolicies = () => {
    ...
    const handleSearch = async () => {
        if (searchTerm.trim() !== '') {
            setIsLoading(true);
            try {
                const response = await axios.get(`/policies/search/${searchTerm}?page=${page}`);
    ...
    return (
        ...
        <h1 className="d-flex align-items-center justify-content-center">Search Policies<Search color="#2A2A84" size={36} className="ms-2"/></h1>
            ...
            </InputGroup>
            {
                results.length > 0 ? (
                    ...
                    {results.map((policy) => (
                        <div className="grid-item d-flex justify-content-center align-items-center" key={policy.PolicyID}>
                            <Link to={`/policy-detail/${policy.PolicyID}`} style={{"textDecoration": "none"}}>
                                <ListGroup.Item className="bg-light nice-border rounded text-dark-purple">
                                <h5 className='text-center text-break fw-light slab-font'>{policy.Property}</h5>
                                </ListGroup.Item>
                            </Link>
                        </div>  
                    ))}
                    ...
                    <div className="">
                        <ListGroup.Item className="bg-light p-5 rounded text-dark-purple">
                            <h3 className='text-center display-6'>No policies Found</h3>
                            <p className='text-center'>
                                Sorry, we could not find any policies that match your search. Please
